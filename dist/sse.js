...
const { ATLASSIAN_SITE, ATLASSIAN_EMAIL, ATLASSIAN_API_TOKEN, MCP_BEARER } = process.env;
...
server.registerTool("confluence.search", {
    ...
    if (!r.ok)
        throw new Error(`search failed: ${r.status} ${await r.text()}`);
    const data = await r.json();
    const out = (data.results || [])
        .filter((h) => h.content?.type === "page")
        .map((h) => ({
        id: h.content.id,
...
server.registerTool("confluence.page", {
    ...
    const r = await fetch(url.toString(), { headers: { Authorization: basic } });
    if (!r.ok)
        throw new Error(`page fetch failed: ${r.status} ${await r.text()}`);
    const data = await r.json();
    const html = data?.body?.storage?.value || "";
    return { content: [{ type: "text", text: html }] };
});
server.registerTool("confluence.attachments", {
    ...
    const r = await fetch(url.toString(), { headers: { Authorization: basic } });
    if (!r.ok)
        throw new Error(`attachments failed: ${r.status} ${await r.text()}`);
    const data = await r.json();
    const out = (data.results || []).map((a) => ({
        title: a.title,
